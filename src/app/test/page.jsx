...
import { BellIcon as BellIconSolid, UserCircleIcon as UserCircleIconSolid, Cog6ToothIcon as Cog6ToothIconSolid, XMarkIcon, CreditCardIcon, InformationCircleIcon } from '@heroicons/react/24/solid'; // Solidアイコンをメインに
...
export default function MyPage() { // コンポーネント名を MyPage に変更 (任意)
  ...
  if (session && session.user) {
    ...
    return (
      <div className="flex flex-col items-center justify-start min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-200 p-4 pb-20"> {/* 全体の背景を薄いグレーに */}
        <h1 className="text-2xl font-bold mt-6 mb-6 text-gray-800 dark:text-gray-100">
          マイページ
        </h1>

        {/* アバターとベルアイコン (通知パネルの基準点も兼ねる) */}
        <div className="relative flex items-center justify-center space-x-6 mb-8" ref={notificationPanelRef}>
          {session.user.image ? (
            <Image src={session.user.image} alt="User Avatar" width={72} height={72} className="rounded-full border-2 border-green-500 shadow-md" priority />
          ) : (
            <div className="w-18 h-18 rounded-full bg-gray-300 dark:bg-gray-600 flex items-center justify-center text-gray-500 dark:text-gray-400 border-2 border-green-500 shadow-md">
              <UserCircleIconSolid className="w-10 h-10" /> {/* アバターがない場合は人型アイコン */}
            </div>
          )}
          <button
            ref={bellButtonRef}
          ...
          {/* 設定(Cog)アイコンはここからは削除し、下のカードに機能を移譲 */}

          {/* 通知パネル (変更なし) */}
          {isPanelOpen && ( <div className={`absolute top-full right-0 sm:left-1/2 sm:-translate-x-1/2 sm:right-auto mt-2 w-80 sm:w-96 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700 z-30 overflow-hidden transition-all duration-200 ease-out origin-top-right sm:origin-top-center ${isPanelClosing ? 'opacity-0 scale-95 pointer-events-none' : 'opacity-100 scale-100'}`}>
              <div className="flex justify-between items-center p-3 px-4 border-b border-gray-200 dark:border-gray-700">
                ...
                <button onClick={handleBellClick} className="p-1 rounded-full hover:bg-gray-100 dark:hover:bg-gray-600 text-gray-500 dark:text-gray-400" aria-label="通知パネルを閉じる"><XMarkIcon className="h-5 w-5"/></button>
              </div>
        ...
        <div className="w-full max-w-md mb-8">
          ...
          <div className="grid grid-cols-3 gap-3 sm:gap-4">
            ...
            <InfoCard icon={UserCircleIconSolid} label="プロフィール" />
        ...
        <div className="w-full max-w-md mb-8">
          ...
          <div className="bg-white dark:bg-gray-700 p-6 rounded-lg shadow-md text-center">
            ...
            <p className="mt-3">
              <a href="#" className="text-xs text-green-600 hover:underline">お支払いヘルプ</a>
            </p>
          </div>
        </div>
        ...
        <div className="w-full max-w-md mb-8">
          ...
          <div className="bg-white dark:bg-gray-700 p-6 rounded-lg shadow-md">
            <div className="flex justify-between items-center mb-3 pb-3 border-b border-gray-200 dark:border-gray-600">
              <span className="text-sm text-gray-600 dark:text-gray-400">ニックネーム</span>
              <span className="font-medium text-gray-800 dark:text-gray-100">{session.user.name?.split(' ')[0] || 'ユーザー'}</span> {/* 名前の最初の部分を仮にニックネームとして */}
            </div>
            <div className="flex justify-between items-center mb-3 pb-3 border-b border-gray-200 dark:border-gray-600">
